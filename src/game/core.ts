...
export function tick(g: MutableGame, dt: number, hit: () => void) { /* keep behavior mostly same */
 ...
 for(let idx=0;idx<s.fighters.length;idx++){const f=s.fighters[idx];if(!f.alive)continue;const c=byId(f.charId);let dx=0,dy=0;if(f.id===g.localPlayerId){const hasAnalog=Math.hypot(i.analogX,i.analogY)>0;dx=hasAnalog?i.analogX:(i.right?1:0)-(i.left?1:0);dy=hasAnalog?i.analogY:(i.down?1:0)-(i.up?1:0);} else { const t=s.fighters.find(x=>x.alive&&x.id!==f.id); if(t){const d=dist(f.x,f.y,t.x,t.y); if(d>c.attackRange){dx=t.x-f.x;dy=t.y-f.y;} if(d<c.attackRange+6) attack(s,f,hit);} }
 if(i.attackPressed&&f.id===g.localPlayerId) attack(s,f,hit); if(i.skillPressed&&f.id===g.localPlayerId) useSkill(s,f); const l=Math.hypot(dx,dy)||1; f.vx=dx/l*c.baseMoveSpeed; f.vy=dy/l*c.baseMoveSpeed; f.x=clamp(f.x+f.vx*dt,f.radius,WORLD.width-f.radius); f.y=clamp(f.y+f.vy*dt,f.radius,WORLD.height-f.radius); if(f.hp<=0&&f.alive){f.alive=false;}}
...
function attack(s: GameState, f: Fighter, hit:()=>void) { const c=byId(f.charId); if(s.time-f.lastAttack<c.attackCooldown) return; f.lastAttack=s.time; s.fighters.forEach(t=>{ if(t.alive&&t.id!==f.id&&dist(f.x,f.y,t.x,t.y)<c.attackRange+t.radius){damage(s,t,c.attackPower); hit();}}); }
