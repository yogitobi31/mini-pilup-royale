...
export interface CharacterDefinition {id: CharacterId; nameKo:string; nameEn:string; role:string; maxHp:number; baseMoveSpeed:number; attackType:'melee'|'ranged'; attackPower:number; attackRange:number; preferredCombatDistance:number; attackCooldown:number; skillPower:number; skillNameKorean:string; skillNameEnglish:string; skillCooldown:number; skillDescription:string; gameplayEffect:string; bodyRadius:number; fallbackColor:string; spritePath:string; portraitPath:string; motif:string; }
export interface Fighter {id:string; charId:CharacterId; x:number;y:number;vx:number;vy:number;radius:number;hp:number;alive:boolean;isPlayer:boolean;playerId?:string;clientId?:string;socketId?:string;isLocal?:boolean;isAI?:boolean;playerType?:'local-human'|'remote-human'|'ai';lastAttack:number;lastSkill:number;facing:{x:number;y:number};targetId?:string;lastHitAt:number;flashUntil:number;fadeUntil:number;status:{slowUntil:number;stunUntil:number;confuseUntil:number;defenseUntil:number;shieldUntil:number;panicUntil:number;speedUntil:number};}
