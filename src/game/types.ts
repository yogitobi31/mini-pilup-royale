...
export interface Fighter {id:string; charId:CharacterId; x:number;y:number;vx:number;vy:number;radius:number;hp:number;alive:boolean;isPlayer:boolean;lastAttack:number;lastSkill:number;facing:{x:number;y:number};targetId?:string;lastHitAt:number;flashUntil:number;fadeUntil:number;status:{slowUntil:number;stunUntil:number;confuseUntil:number;defenseUntil:number;shieldUntil:number;panicUntil:number;speedUntil:number};}
